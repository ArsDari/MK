...
int main(void)
{
	...
	while (1)
	{
		segBuffer[0] = segments[counter / 1000];
		segBuffer[1] = segments[(counter / 100) % 10];
		segBuffer[2] = segments[(counter / 10) % 10];
		segBuffer[3] = segments[counter % 10];
...
{
	uint8_t p = segBuffer[whichDigit];
